...
/*
translate([0,0,5+z_bearing_block_length])z_carriage(arm=0);
z_carriage(arm=1);
...
module z_carriage(arm=1){
	...
	difference(){
		...
		translate([z_bearing_block_width/2, 0,-0.1]) cylinder(d=z_bearing[1]+0.2, h=z_bearing_block_length+0.2, $fn=30);
			...
			translate([0,0,z_bearing_block_length-20]){
				...
				translate([z_bearing_block_width+6,-0.1,0]) rotate([-90,0,0]) cylinder(d=frame_screw, h=z_bearing_block_depth+0.2, $fn=60);
				translate([bed_arm_length-13,-0.1,0]) rotate([-90,0,0]) cylinder(d=frame_screw, h=z_bearing_block_depth+0.2, $fn=60);
